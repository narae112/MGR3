...
//    @Query("Select new com.MGR.dto.ReservationDtlDto(rt.id, t.name, t.price, rt.ticketCount, rt.visitDate, rt.reservationDate, rt.reservationStatus) " +
...
//    )
//    List<ReservationDtlDto> findReservationDtlDtoList(@Param("reservationId")Long reservationId);
    ...
    @Query("select rt from ReservationTicket rt " +
            "where rt.member.email = :email " +
            "order by rt.reservationDate desc"
    )
    List<ReservationTicket> findReservations(@Param("email") String email, Pageable pageable);
